...
digest_stats: [IF|3.8（J. Chromatogr. A・JCR2024・Q1）, 生薬|豨薟草（きけんそう・祛風湿）, 試料|20バッチ, 手法|5波長融合指紋＋比率定量指紋法（RSQFM）＋UV/FTIR量子化指紋, 抗酸化|DPPH指紋-効能相関でクロロゲン酸・カフェ酸・ルチンを同定]
digest_points: [薬典が単一マーカー（キレノール）だけの豨薟草に対し5波長融合指紋（29共通ピーク）＋比率定量指紋法（RSQFM）で多次元の品質評価系を構築, 比率ri=試料/標準ピーク面積で評価するので補正係数の偏りがなく参照指紋定量法（RFPQM）で標準品消費を大幅削減（RD<6%でSCMと一致）, HPLC/UV/FTIRの3指紋を等重み融合し20バッチを5等級に格付けしDPPH相関でクロロゲン酸・カフェ酸・ルチンを抗酸化成分と特定]
digest_chart_label: 品質格付けの推移（HPLC単独→3手法融合）
digest_chart: [HPLC単独の等級数|8, 3手法融合後の等級数|5]
summary: 薬典が単一マーカー（キレノール）でしか規定していない生薬「豨薟草」の品質を、HPLC 5波長融合指紋（FWFFP・29共通ピーク）＋「比率（試料/標準）」で評価する系統的比率定量指紋法（RSQFM）＋UV/FTIR量子化指紋＋DPPH抗酸化活性を統合して多次元評価。RSQFMは各ピークの「試料/標準」面積比 ri=xi/yi を計算指標とし、比率マクロ定性類似度 Smr と比率マクロ定量類似度 Pmr で全バッチを8等級に格付けする。20バッチのSHをFWFFPで評価し、不確かさ理論で信頼性を数値化（全U値<0.2）。参照指紋定量法（RFPQM）でRFPを1回定量するだけでCA/CAA/RN/KLの4成分を求め、標準曲線法（SCM）とRD<6%で一致（標準品消費を削減）。UV・FTIRを量子化指紋に変換し、HPLC/UV/FTIRの3指紋を等重み融合して5等級に格付け。指紋-効能（DPPH）相関でクロロゲン酸・カフェ酸・ルチンを抗酸化寄与成分と特定した。
---
...
![図1. ROFPの4レベルの全体定量類似度から成る六面体（hexahedra）。](assets/siegesbeckiae-herba-ratio-fingerprint-antioxidant-qc/fig1.png)
...
![図2. (A) 条件別のHPLC試料TQ値；(B) 3Dクロマトグラム；(C) 5波長融合過程；(D,E) ROFP生成過程；(F) 20バッチ試料のROFP。](assets/siegesbeckiae-herba-ratio-fingerprint-antioxidant-qc/fig2.png)
...
![図3. (A) 5単一波長と融合後の20バッチRSQFM評価結果；(B) クラスター解析ヒートマップ；(C) スコア散布図；(D) 指紋-効能関係のVIPプロット（赤：VIP>1）。](assets/siegesbeckiae-herba-ratio-fingerprint-antioxidant-qc/fig3.png)
...
![図4. (A) HPLCの試料・標準液クロマトグラム；(B) 20バッチSHの4成分含量；(C-F) 2測定法によるCA・CAA・RN・KLのフィッティング曲線。](assets/siegesbeckiae-herba-ratio-fingerprint-antioxidant-qc/fig4.png)
...
![図5. (A) UV特性吸収ピーク；(B) FTIR特性吸収ピーク；(C) 異なる併合点のUV量子化スペクトル；(D) 異なる併合点のFTIR量子化スペクトル。](assets/siegesbeckiae-herba-ratio-fingerprint-antioxidant-qc/fig5.png)
...
![図6. (A,B) HPLC・UV・FTIRで評価したSmr・Pmrのレーダーチャート；(C) Pmr-FWFFP・Pmr-UV・Pmr-FTIR・Pmr-Fの変動（箱ひげ図）。](assets/siegesbeckiae-herba-ratio-fingerprint-antioxidant-qc/fig6.png)
...
![図7. (A) IC50値；(B) スコア散布図；(C) 訓練モデルのY実測vs予測；(D) 検証モデル；(E) 標準化回帰係数プロット；(F) 指紋-効能関係のVIPプロット（赤：VIP>1）。](assets/siegesbeckiae-herba-ratio-fingerprint-antioxidant-qc/fig7.png)
