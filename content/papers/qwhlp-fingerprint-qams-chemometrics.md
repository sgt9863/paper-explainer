...
date: 2026-07-04
published: 2025
tags: [方剤・中成薬, 指紋分析, QAMS, ケモメトリクス, QC・品質評価]
...
- 注意点として、本研究は原題に "Quantitative Analysis of Multi-Index Components" とあるように**複数成分をそれぞれ独立に外部標準法（検量線）で定量する方式**であり、姉妹論文で扱われる QAMS（一マーカー多成分定量、内部基準物質の相対補正係数から他成分濃度を換算する手法）とは手法が異なる。本文中にも「QAMS」「相対補正係数（RCF）」といった記述は登場しない。本ページのタグには運用上の一貫性のため「QAMS」が付与されているが、内容としては「多成分定量」に近い点にご留意いただきたい。
