...
表I（因子水準）・表II（DSD実験）・表III（回帰係数）を保持。回帰式（Eq.3-8）は本文の通り。図（Fig.1 石川図とFig.5 材料温度はベクター図のため割愛。Fig.2-4・6-8を本文に埋め込み）。
-->
...
![図3. 原料利用率に対する各独立変数の影響を示す応答等高線図（a-f）。](assets/xiaochaihu-qbd-fluid-bed-granulation-dsd/fig3.png)
...
![図8. 異なる操作条件で製造した顆粒中のバイカリン含量。](assets/xiaochaihu-qbd-fluid-bed-granulation-dsd/fig8.png)
