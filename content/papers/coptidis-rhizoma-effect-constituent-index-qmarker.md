---
title: 効果-成分指数（ECI）という統合的・薬効指向のQ-markerによる生薬の品質規格の高度化 — 黄連（Coptidis Rhizoma）を例として
...
date: 2026-07-05
published: 2018
tags: [生薬, Q-marker, QC・品質評価, 多成分定量, 薬効・薬理, スペクトル効果, HPLC・UPLC]
digest_tagline: 各成分の活性で重み付けした「効果-成分指数(ECI)」を黄連の薬効指向Q-markerとし、単一成分QCや外観評価が捉えられない薬効ベースの品質等級を判別
digest_stats: [IF|7.9（Phytomedicine JCR2024 / Clarivate・要確認）, 対象|黄連(Coptidis Rhizoma), マーカー|プロトベルベリン系5成分（BER・EPI・COP・PAL・JAT）, 薬効|抗菌（赤痢菌）・抗腫瘍（HepG2）, 指標|ECIS（抗菌）・ECIH（抗腫瘍）, 相関|ECIS-RIS R=0.987／ECIH-1/EC50 R=0.873（ともにP<0.01）]
digest_points: [各成分の活性（IC50・EC50）の逆数で「効果の重み」を決め含量に掛け合わせる。含量だけでなく「効いている度合い」で重み付けした薬効指向のQ-marker=ECIを構築, 抗菌活性の順位はCOP>BER>EPI>JAT>PAL・抗腫瘍はBERが最強。ベルベリン(BER)偏重や非マーカー成分JATの寄与見落としの危険を実データで指摘, 外観（官能）評価とベルベリン含量は薬効ベースの品質等級を区別できなかったがECISとECIHは3等級を有意に分離。用途別（抗菌用／抗腫瘍用）の個別品質評価を可能にする]
digest_chart_label: 黄連10バッチ中のベルベリン(BER)含量の幅（%・Fig.4aより読み取り）
digest_chart: [最低バッチ|18.9, 中位バッチ|21.2, 最高バッチ|24.6]
summary: 抗菌・抗腫瘍という異なる薬効に用いられる黄連（Coptidis Rhizoma）について、5つのプロトベルベリン系アルカロイドの含量（HPLC）と各成分の活性（微量熱測定によるIC50・MTTによるEC50）を統合し、活性で重み付けした「効果-成分指数（ECI）」を薬効指向のQ-markerとして提案した論文の全訳（＋訳者補足）。ECIが単一成分・官能評価では捉えられない薬効ベースの品質等級を判別・予測できることを示す。IC50/EC50・ECIモデル式・相関係数・HPLC条件まで網羅。
---
...
![Fig. 1. 黄連で調べた5成分（プロトベルベリン系アルカロイド）の化学構造。左：BER と COP の骨格（R₃・R₄ の置換基が BER は CH₃/CH₃、COP は −CH₂−（メチレンジオキシ））。右：PAL・JAT・EPI の骨格（R₁〜R₄ の置換基が各成分で異なる）。](assets/coptidis-rhizoma-effect-constituent-index-qmarker/fig1-structures.jpeg)
...
![Fig. 2. 異なる濃度のアルカロイドが影響した赤痢菌（S. dysenteriae）の HFP-t 曲線（熱流パワー-時間曲線）。](assets/coptidis-rhizoma-effect-constituent-index-qmarker/fig2-hfp-curves.jpeg)
...
![Fig. 3. 黄連中アルカロイドの HepG2 細胞に対する細胞毒性。測定は 3 連で行い、エラーバーは平均の標準誤差を表す。](assets/coptidis-rhizoma-effect-constituent-index-qmarker/fig3-cytotoxicity.jpeg)
...
![Fig. 4. (a) 黄連 10 バッチの 5 成分の含量（%）、(b) 10 バッチの ECIS と ECIH。測定は 3 連で行い、エラーバーは平均の標準誤差。⁎⁎ P < 0.01（対応する ECIS 値との比較）。](assets/coptidis-rhizoma-effect-constituent-index-qmarker/fig4-contents-eci.jpeg)
...
![Fig. 5. (a) 異なる濃度の黄連エキスが影響した赤痢菌の HFP-t 曲線、(b) 黄連エキスの HepG2 細胞に対する細胞毒性。HepG2 への阻害効果の測定は 3 連で行い、エラーバーは平均の標準誤差。](assets/coptidis-rhizoma-effect-constituent-index-qmarker/fig5-extract-inhibition.jpeg)

![Fig. 6. (a) ECIS 値と RIS の関係、(b) ECIH 値と 1/EC50 の関係。](assets/coptidis-rhizoma-effect-constituent-index-qmarker/fig6-correlation.jpeg)
...
![Fig. 7. (a) 赤痢菌への阻害効果に基づく LDA による黄連試料の分類結果、(b) 官能品質スコア、(c) 黄連中の BER 含量、(d) 抗菌効果ベースの等級ごとの試料の ECIS。](assets/coptidis-rhizoma-effect-constituent-index-qmarker/fig7-lda-antibacterial.jpeg)

![Fig. 8. (a) HepG2 細胞への細胞毒性に基づく LDA による黄連試料の分類結果、(b) 官能品質スコア、(c) 黄連中の BER 含量、(d) 抗腫瘍効果ベースの等級ごとの試料の ECIS。](assets/coptidis-rhizoma-effect-constituent-index-qmarker/fig8-lda-antineoplastic.jpeg)
...
![Fig. 9. (a) 優・良・可の官能評価結果をもつ黄連試料、(b) 黄連 16 バッチの 5 活性成分のヒートマップ。](assets/coptidis-rhizoma-effect-constituent-index-qmarker/fig9-organoleptic-heatmap.jpeg)
...
- **示された事実の要点**: 抗菌活性順位 **COP>BER>EPI>JAT>PAL**、抗腫瘍では **BER 最強・JAT 最弱**。BER 偏重・非マーカー成分（JAT）の寄与見落としの危険を数値で提示。**官能評価もベルベリン含量も薬効ベースの等級を区別できなかった**が、ECIS・ECIH は 3 等級を有意に分離（ECIS-RIS 相関 R=0.987、ECIH-1/EC50 相関 R=0.873、いずれも P<0.01）。
...
- **他の解説との関係**: 本ワークスペースには同じ「効果-成分指数（IEQFM/ECI）」を複方甘草片に応用した別論文の解説（`clt-fingerprint-ieqfm-fusion`）や、Q-marker 概念の総説（`qmarker-new-concept-liu2017` ほか）がある。本稿はその ECI 手法の原型に近い、単一生薬（黄連）×2 薬効での実証例として読むと位置づけが分かりやすい。
