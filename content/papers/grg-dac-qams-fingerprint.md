---
title: ドイツ薬用規範(DAC)準拠による天麻(Gastrodiae Rhizoma)顆粒の多元的品質評価
...
level: practitioner
date: 2026-07-07
tags: [漢方, 生薬, QC, 天麻, DAC, QAMS, HPLC指紋, TLC, システム適合性]
digest_tagline: ドイツ薬用規範(DAC)準拠のTLC＋QAMS＋HPLC指紋分析で天麻(Gastrodia)顆粒15バッチを多元的に品質評価
digest_stats: [IF（2025）|5.4, 手法|TLC＋QAMS＋HPLC指紋（DAC準拠）, マーカー|6成分（ゲニン他）, ゲニン含量|1.381–5.312%, 精度|RSD<2.6%, HPLC分離度|Rs>4.5（パリシンB/C）]
digest_points: [TLC(SST:ゲニン/パリシンB分離)＋QAMS(ゲニンを内標に6成分同時定量)＋HPLC指紋(58ピーク・コサイン類似度/PCA/HCA/レーダー図)を統合したDAC準拠の多元的評価戦略, QAMSと外標準法(ESM)で有意差なし(P>0.05／R²=0.999-1.000)・ゲニン含量1.381-5.312%など6成分を精度よく同時定量, 中国国家標準(CNS)と比較しシステム適合性・多成分特性評価をより重視／S5・S7・S10・S14の一部バッチに組成偏差を検出]
summary: ドイツ薬用規範(DAC)準拠の分析戦略（TLC同定＋QAMS多成分同時定量＋HPLC指紋）で市販の天麻(Gastrodia elata)顆粒15バッチを多元的に品質評価し、中国国家標準(CNS)と比較した論文の全訳（＋訳者補足）。検量線・LOD/LOQ・RSD・回収率・含量など定量データを網羅。
---
...
![Figure 1. GRG試料のTLC同定結果。(A)S1〜S8および(C)S9〜S15を日光下、(B)S1〜S8および(D)S9〜S15を310nm紫外光下で観察。a: 標準溶液a、b: 標準溶液b、c: 標準溶液c。1: ゲニン、2: パリシンB。](assets/grg-dac-qams-fingerprint/fig1-tlc-identification.jpg)
...
![Figure 2. (A)6マーカー成分の構造式、(B)ブランク対照・標準溶液・検液のHPLCクロマトグラム、(C)機器精度・再現性・室内再現精度のRSD値、(D)6マーカー成分の36時間にわたる安定性、(E)6マーカー成分の回収率。ピーク番号: 1.ゲニン、2.未知成分、3.p-ヒドロキシベンジルアルコール、4.パリシンE、5.パリシンB、6.パリシンC、7.パリシンA。](assets/grg-dac-qams-fingerprint/fig2-structures-chromatogram-validation.jpg)
...
![Figure 3. (A)各成分をIRとした場合の6成分のRCF、(B)ゲニン、(C)p-ヒドロキシベンジルアルコール、(D)パリシンE、(E)パリシンB、(F)パリシンC、(G)パリシンAをIRとした場合のQAMSとESM間RSD。](assets/grg-dac-qams-fingerprint/fig3-rcf-boxplots.jpg)
...
![Figure 4. (A〜F)ESMとQAMSの含量値を比較した相関散布図、(G)GRG試料中6成分の含量。](assets/grg-dac-qams-fingerprint/fig4-esm-qams-correlation-content.jpg)
...
![Figure 5. (A)15バッチのGRG試料の特徴的HPLC指紋、(B)全ペアバッチ比較のコサイン類似度係数、(C)全検出ピークのバッチ間ピーク面積ヒートマップ。](assets/grg-dac-qams-fingerprint/fig5-hplc-fingerprint-similarity-heatmap.jpg)
...
![Figure 6. 15バッチのGRG試料のケモメトリクス解析。(A)PCAスコアプロット、(B)HCAデンドログラム、(C)レーダープロット。](assets/grg-dac-qams-fingerprint/fig6-pca-hca-radar.jpg)
...
- **本研究の「DAC準拠」は法的な登録基準そのものではない**点に注意。著者らは明示的に「DAC/EP様式の期待に整合する分析要素の採用」であり、GRGに対する法的拘束力のある基準の直接実装ではないと述べている。国際展開時の規格設計の"たたき台"として位置づける。
- **他施設で追試する際の要確認点**: ①TLC SSTは単一濃度の合否判定ではなく、**ゲニン/パリシンBという構造既知の2成分間のRf分離**で評価する設計。②HPLC SSTは理論段数基準（CNSのUHPLC・N≥5000）ではなく、**パリシンB/C間の分離度（本研究でRs>4.5、要求は>1.5）**で評価する設計。両者は評価軸が異なるため、CNS適合とDAC適合は別々に確認する必要がある。
