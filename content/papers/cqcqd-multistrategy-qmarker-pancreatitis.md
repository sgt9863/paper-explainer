...
![図2. CQCQD 10バッチの22化学マーカーの指紋とケモメトリクス解析。(A)UHPLC結果、(B)含量分布、(C)PCA、(D)ヒートマップ。](assets/cqcqd-multistrategy-qmarker-pancreatitis/fig2.png)

![図3. UHPLC-HRMSによる「体内移行」特性に基づくQ-marker screening。(A)ワークフロー、(B)血漿中プロトタイプ化合物のクロマトグラム(CQCQD正負イオン/血漿正負イオン)。](assets/cqcqd-multistrategy-qmarker-pancreatitis/fig3.png)

![図4. CQCQDで同定された22化合物の化学構造。](assets/cqcqd-multistrategy-qmarker-pancreatitis/fig4.png)

![図5. 成分-薬効相関に基づくQ-marker一次スクリーニング。(A)「生薬-成分-標的-経路」ネットワーク、(B)免疫染色像、(C)壊死率、(D)相関行列。](assets/cqcqd-multistrategy-qmarker-pancreatitis/fig5.png)
...
![図7. 22化学マーカーの含量プロファイル(A: 低用量/高用量比較、B: 拡大図)。](assets/cqcqd-multistrategy-qmarker-pancreatitis/fig7.png)
