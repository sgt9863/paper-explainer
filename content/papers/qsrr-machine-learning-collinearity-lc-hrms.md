...
図Fig.1-3を本文に埋め込み。SI（ST1-4, SF1-14）は原文参照。
-->
...
![図1. 本研究の全体手法のフローチャート（LCデータセット→分子記述子の計算→記述子の絞り込み→回帰アルゴリズム→入れ子交差検証→QSRRモデル予測→評価）。](assets/qsrr-machine-learning-collinearity-lc-hrms/fig1.png)
...
![図2. 各データセット・各モデルにおけるQSRR予測モデルの性能（MedAE等）。](assets/qsrr-machine-learning-collinearity-lc-hrms/fig2.png)
...
![図3. Kruskal-Wallis H検定によるp値のヒートマップ（144ケース。緑枠は統計的有意差のケース）。](assets/qsrr-machine-learning-collinearity-lc-hrms/fig3.png)
