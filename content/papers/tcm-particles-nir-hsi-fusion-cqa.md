...
digest_stats: [IF（2024）|3.15, 対象|中成薬顆粒(TCMP)・流動層造粒, バッチ|20＋外部検証10(40サンプル), 手法|FT-NIR＋Vis/NIR-HSI融合(3段階), 予測CQA|安息角・粒度(D50)・水分含量]
digest_points: [FT-NIRは粒度・水分含量の予測でVis/NIR-HSIより高精度、逆にVis/NIR-HSIは安息角(流動性指標)の予測で優位, 低位(LLF)・中位(MLF)・高位(HLF)の3段階融合を比較し高位融合が3指標すべてで最高精度を達成(RPD: 安息角5.090・粒度18.893・水分30.891), 外部検証セット(10バッチ・40サンプル)でも高位融合モデルは高精度を維持しPAT実装可能性を示した]
