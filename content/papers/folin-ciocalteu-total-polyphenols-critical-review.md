...
分析校正は実験信号とアナライト濃度の関係を確立する、あらゆる分析法開発に共通する必須ステップである[57]。残念ながら、報告されたFC研究の多くはMV（方法バリデーション）に関するそれ以上の情報を伴わずに研究を終えている。例えば、選定した50論文のうち、GAを優先化学標準としたMV研究をカバーしていたのはわずか16件だった。表2・表3はこれら16件のMVの性能パラメータをまとめたもの（原著Table 2・Table 3）。MVは、あらかじめ確立されたシグナル対濃度関係を用いて分析法のさまざまな性能パラメータを得るために役立つという点で等しく重要であることを強調しておきたい。これまでTPC定量に用いられたバリデート法の情報をまとめた体系的レビューは存在しない。まず理論的な観点から、MVに関するガイドラインや文献は数多く存在する。性能パラメータの面では大筋の合意があるものの、命名法・採用手順・実務上の指針については有意な差異が存在する[58]。残念ながら、収集した研究においてFCアッセイの適用性評価の基礎として用いたガイドラインが明記されることは稀であり、例外はAnvisa[59]・AOAC International[60]・Eurachem[61]・ICH[62]（最多引用）・ISO 5725-2[63]・USFDA[64]のみだった。

以下の節では、FCアッセイのバリデーションに関する既往研究を、選択性・直線性・精度・真度・検出限界・定量限界・堅牢性の結果に従ってまとめる。

### 3.1 選択性

分析法は、干渉なく目的アナライトのみを測定するとき選択的とみなされる。実際、大半の論文でこのパラメータは評価されなかった。これはFCアッセイが非選択的な方法であることが既に知られており、FCR使用時に吸光度シグナルへ干渉しうる化合物が多いためである。この点を改善するため、Sánchez-Rangelらは複数の方法論を評価し、最終的に抽出試料中のアスコルビン酸含量に応じてTPC値を補正することを提案した[65]。

しかし本レビューに含めた論文の一部は選択性試験を実施していた。Kupinaらは糖類（フルクトース・グルコース・スクロース）を試験し正の効果が無いことを報告した[30]。同様にMargraftら・Maらはアスコルビン酸・グルコース・メタ重亜硫酸カリウムを評価し、いくつかの正の効果を記述した[23,37]。最後に、Lawagらも一部の還元糖による正の影響を実証している[54]。

### 3.2 直線性

FCアッセイで検量線を作成するのに内部標準は不要であり、通常GAを化学標準とする外部校正が報告される。一般にMVガイドラインは、分析法が適用される対象と同一のマトリックス中で検量線を作成することを推奨する。したがって本来は、マトリックスを含む試料／含まない試料いずれで校正手順を行ったかを記述すべきである。しかし、収集したバリデーション研究のうちマトリックスの影響を含めていたのはごく一部にとどまる[27,29,39,54]。加えて、検量範囲（実試料の予想濃度に応じた）・校正水準・反復測定回数・短時間区間内での校正系列数を記述することが重要である。全体として、検量線は5〜7水準と多様な反復測定数で作成されたと報告された。しかし校正系列数を報告していたのはKupinaらの研究のみだった[30]。この意味で、校正水準数や曲線の系列数を減らすことは避けるべきであり、それはFC法の信頼性に直接影響する。一方、収集した論文の大半は決定係数（R²）値を検量関数の許容基準として報告していた。しかし、既往の指摘によればR²値は回帰の評価には不適切である[66]。

### 3.3 検出限界（LOD）・定量限界（LOQ）

LODとLOQは理論的にも実験的にも議論の多い2つの性能パラメータである。両パラメータを推定するアプローチは複数存在する。本レビューでの論文収集では、選定した手法論に関する情報が該当セクションに含まれていないことがあった。含まれている場合も、ブランク試料の標準偏差と検量線からの算出のみが記述されていた。報告値は広く変動し、選択した検量範囲と連動していた。LOD・LOQの極端な低値・高値の例として、0.006および0.019 mg/L[17]対14.2および42.9 mg/L[37]が報告されている。いずれにせよ、これらの限界値は理論的に定義されたものとみなすことができる。逆に、許容できる精度・真度を伴う定量的な決定を保証できる最小濃度によって、実験的に評価することが望ましい。LOQとして報告された値が評価された検量範囲より低いこともあった[17,22,32,56]。

### 3.4 精度

精度は、特定条件下で実験手順を繰り返し実施して得られた値どうしの近さを反映する。命名法の観点から、精度の結果は2水準——日内（併行精度）と日間（室内再現精度）——で表現すべきである。室内再現精度は単一ラボでMVを実施する場合に用いるべき用語であり、逆に再現性（reproducibility）は室間共同試験以外では用いるべきでない点に留意されたい[58]。

記述がある場合、示された精度基準は1.5%〜10% RSDの範囲だった。水準数・測定回数・記述された精度値は収集した方法論間で広く異なっていた。驚くべきことに、報告された%RSD値は吸光度値（AU）・抽出溶液中濃度（mg/L）・原試料中濃度（%w/w）という異なる概念に基づいていた。日内精度の値が日間精度より高い場合もあった[23]。さらに、標準校正溶液を用いるか添加した実試料（spiked native samples）を用いるかによって、このパラメータの評価方法が大きく異なる点に注意が必要である。両者の結果を比較すると、実試料の場合にこのパラメータはより高く、正しく評価されていないように見える[30]。しかし、特に抽出物調製もラボで行われる場合、こうしたケースでの結果の信頼性はより高い。

### 3.5 真度

この用語は、多数の試験結果の算術平均と真の値または許容参照値との近さを指す。残念ながらこの語は正確度（accuracy）・回収率（recovery）といった他の用語としばしば混同される。実際、選定したFCバリデーション論文のいずれも「真度（trueness）」という語を用いていなかった。好まれた表現は「回収率」だったが、この語は事前の抽出段階の要否に応じて絶対回収率・見かけの回収率という2つの異なる文脈で使われうる点に注意が必要である[58]。

この評価方法については、精度の研究と同様、試料は理想的には校正手順と同じ由来ではなく独立した供給源から得るべきである。さらに試料は可能な限り未知試料に近いものであるべきである。しかし選定論文の一部はこの情報を含んでいなかった[6,28]。単純な校正標準を用いた報告も時折見られた[17,22,56]。より多く報告されたのは、既知量のアナライトを試料またはその抽出物に添加するスパイク法である。注目すべきは、Kupinaらが標準校正溶液とマルトデキストリン中のブドウ種子抽出物を代表的キャリアとして用いて真度を評価した点である[30]。濃度水準・反復測定は研究間で大きく異なっていた。記述がある場合、80〜110%または85〜115%の見かけの回収率基準が満足できるものとみなされた。

### 3.6 堅牢性

このパラメータの目的は、結果に大きく影響する実験因子を見出すことである。実験条件の意図的な変動を通じて、外部要因に対する手順内の誤差の安定性が示される。本レビューに含めた論文のうち2件が2因子の堅牢性試験を実施していた。まずPereiraらはFCR（45〜55% v/v）とNa2CO3（4.5〜5.5% v/w）の影響を検討した[32]。同様にMartinsらはNa2CO3濃度と反応時間の変動を評価した[44]。さらにKupinaらは、伝統的なYouden and Steiner法を用い、抽出物の重量・加熱装置と温度・2つの反応時間・2つの試薬（FCRとアルカリ）量という7つの主要因子に基づく8回の実験からなる堅牢性試験を実施した[67]。選択したいずれの因子も得られたTPC含量に有意差をもたらさなかったと報告されたが、実際に記述された結果は73.86〜93.75%(w/w)の範囲で変動していた[30]。

**表2. FCアッセイのMV性能パラメータ（原著Table 2、選択性・直線性・LOD/LOQ）**（水準=Levels、反復=Replicas、系列=Series。AA=アスコルビン酸、FRU=フルクトース、GLU=グルコース、PoMeBis/SoMeBis=メタ重亜硫酸カリウム、SUC=スクロース、TYR=チロシン、Conv=従来型吸光度測定、MPR=マイクロプレートリーダー、「—」=原文空欄）

| 文献 | ガイドライン | 選択性(干渉物質) | 直線性(mg/L) | 水準/反復/系列 | 判定基準 | LOD(mg/L) | LOQ(mg/L) | 手法 | 堅牢性 |
|---|---|---|---|---|---|---|---|---|---|
| [7] | ISO 5725-2 | — | 10–50 | — | R² | — | — | — | — |
| [14] | AOAC International | — | 1–8 | 5水準 | R²/SD resid | 0.07 | 0.11 | ブランク(10) | — |
| [17] | AOAC International | — | 0.25–10 | 7水準(a) | R²/SD resid | 0.006 | 0.019 | ブランク(10) | — |
| [22] | — | — | 10–200 | 5水準 | R² | 0.74 | 2.24 | 校正曲線 | — |
| [23] | Eurachem & USFDA | AA, GLU, PoMeBis | 0–180 | 9水準/3反復 | R² | 0.25 | 0.82 | 校正曲線 | — |
| [25] | — | — | 1.25–120 | — | R² | 6.03 | 16.21 | ブランク(10)／低濃度試料 | — |
| [27] | 国際ガイドライン(NR) | マトリックス効果 | 0.5–9 | 6水準/6反復 | R²/r | — | — | — | — |
| [28] | ICH | — | 15–75 | 4水準/3反復 | R² & Mandel | — | — | — | — |
| [29] | Anvisa & ICH | マトリックス効果 | 2–8 | — | R² & 残差 | — | — | 校正曲線 | 照度・機器 |
| [30] | — | FRU, GLU, SUC | 40–200 | 5水準/2反復/4系列 | R² & 残差 | — | 40 | 完全試験 | — |
| [32] | ICH | — | 2.5–50 | 6水準/3反復 | R² & LOF | 0.195 | 0.591 | 校正曲線 | FCR・BM |
| [37] | — | AA, GLU, SoMeBis, TYR | 43–500 | — | R² | 14.2 | 42.9 | 校正曲線 | — |
| [39] | — | マトリックス効果 | 10–100 | 6水準/6反復 | R² | 5.03 | 15.24 | 校正曲線 | — |
| [49] | — | — | 0–1 | 6水準/3反復 | R² | 0.1024 | 0.3412(a) | 校正曲線 | — |
| [54] | ICH | 各種化合物・マトリックス効果 | 30–90 | 7水準/3反復 | R² | 2.55 | 7.74 | 校正曲線 | 時間・BM |
| [56] | — | — | 10–60 | 6水準 | R² | Conv:0.58／MPR:1.19 | Conv:1.93／MPR:3.98 | 校正曲線 | — |

**表3. FCアッセイのMV性能パラメータ（原著Table 3、精度・真度）**（試料数=Samples、水準=Levels、反復=Replicas、日数=Days。Repe=併行精度、Repro=室内再現精度、Intra=日内、Inter=日間、A&R=正確性・回収率、STD=標準品、SAMP=試料。「—」=原文空欄）

| 文献 | マトリックス | 精度(%RSD) | 標準/試料 | 試料数/水準/反復/日数 | 判定基準(%) | 真度(回収率%) | 標準/試料 | 試料数/水準/反復/日数 | 判定基準(%) |
|---|---|---|---|---|---|---|---|---|---|
| [7] | 茶 | 1.16–1.53／4.64–7.38 | 試料 | — | — | — | — | — | — |
| [14] | 尿 | 0.6–7.2 | 添加 | 5水準 | 7.30 | (R)82.5–105.7 | 添加 | 5水準 | 80–110 |
| [17] | トマト品種 | Intra:0.04–0.10／Inter:0.05–0.12 | 標準品 | 7水準(a) | — | (R)Intra:90.6–103.7／Inter:91.9–105.1 | 標準品 | 7水準(a) | — |
| [22] | ブドウ種子・リンゴ | Repe:2.1–3.6／Repro:2.9–6.1 | 試料 | 5反復-1日／5反復-別日 | 10 | (A&R):87.8–100.3 | 試料 | 5反復 | — |
| [23] | ジュース・茶 | Intra:6.65／Inter:1.13 | 標準品(100mg/L) | 1水準/10反復／1水準/3反復/3日 | 10 | (A&R):93.5–102.9 | 標準品 | 1水準/10反復 | 85–115 |
| [25] | 乳 | Repe/Intra:4.0／Repro/Inter:6.8 | 試料 | 6試料 | — | (R)85.4–99.3 | 添加／試料 | 3水準／(80-100-120) | — |
| [27] | カシューナッツ殻 | Repe:0.17–0.73 | 標準品 | 5水準/6反復 | — | (A&R):99.7–105.1 | 添加 | 3水準(a) | 85–115 |
| [28] | プーアル茶TPC | Repe:1.2–1.4／Repro:<1.5 | 試料／試料(a) | 3試料/3反復／3試料/3日 | — | – | — | — | — |
| [29] | 樹皮・莢 | Repe:0.91–1.28(a)／室内精度:0.57–2.15(a) | 試料 | 6試料／3試料/2日 | 5 | (A&R):96.84–103.13 | 添加 | 3水準 | — |
| [30] | 各種抽出物 | Intra:1.96–7.47／Inter:4.39–13.77 | 試料 | 5試料/4反復/4日 | <7-9 | (R)33.56–56.6/91.0–93.7/94.8–100.4／(R)90.3–175/91–119/109-120 | 添加／マルトデキストリン | 3水準(a)/3反復/3日 | 80–110 |
| [32] | 食品試料 | Intra:0.06–0.88(b)／Inter:0.06–3.28(b) | 標準品 | 3水準/3反復／3水準/3日 | — | (A&R):93.3–104.3 | 添加 | 15試料/1水準 | — |
| [37] | 果汁・シードル | Intra:0.66 | 標準品 | 1水準/10反復 | — | (A&R):102.9 | 添加 | 1水準/10反復 | — |
| [39] | トウダイグサ科植物 | システム:0.33／Intra:0.52–0.63／Inter:1.31–1.84 | 標準品 | 1水準/10反復／3水準/3反復/1日／3水準/3反復/3日 | <2 | (A&R):102.36 | 添加抽出物 | — | — |
| [49] | ホウレンソウ | Repe:1.79(b) | 試料 | 1試料/7反復 | — | (A&R):81.50–85.66 | 添加試料 | 2水準 | 80–110 |
| [54] | 蜂蜜 | Intra:0.51–0.63／Inter:0.48–0.70 | 標準品 | 3水準/3反復/1日／3水準/3反復/3日 | ICH | (A&R):99.1–100.2 | 添加／試料 | 3水準/3反復 | ICH |
| [56] | 植物抽出物 | Intra:1.91／Inter:1.46 | 標準品 | 3水準/4反復 | 5 | (A&R):101.3–105.9 | 標準品 | 3水準/3反復 | — |

> 補足: 表2・3の上付き注（a）は原著で「実験データの矛盾（範囲）」、（b）は「実験データの矛盾（単位）」と注記されている箇所——著者ら自身が元論文の記載に不整合があったと明記している値であり、本サイトでもその注記を保持した。

**全体として、報告された性能パラメータは選択した実験因子と強く連動しており広く変動する**。校正範囲は0〜1 mg/Lから最大1250 mg/Lまで、直線点数は4〜9水準、反復は2〜10回。堅牢性は主にブランク（10）や校正曲線・マトリックス効果・機器・照度で評価された。LOD/LOQも0.006/0.019 mg/Lから14.2/42.9 mg/Lまで実験因子に依存して大きく変動する。
