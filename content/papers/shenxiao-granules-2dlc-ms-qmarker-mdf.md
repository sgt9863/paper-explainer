...
summary: 糖尿病腎症治療の8生薬院内製剤「腎消顆粒（SXG）」の品質管理を、オフライン直交HILIC×RP 二次元LC（理論ピーク容量7161＝従来比58倍）＋高分解能Q-TOF-MS＋質量欠損フィルタ（MDF）で実現。260成分を同定（36を標準品で確定、うちMS-DIALが250・GNPS分子ネットワークが38を注釈）し、薬局方適合・腎保護薬理・共通指紋ピークの3次元枠組みで12のQ-markerを選定、UPLC-MS/MS（MRM）で9バッチを定量した（回収率84.73–119.09%、RSD<5%、LOD 0.02–6.56 ng/mL）。丹参フェノール酸（サルビアノール酸B）にバッチ間変動が見られ原料・工程管理の課題を指摘。「多次元分離-計算注釈-階層的Q-markerスクリーニング」の枠組みは複雑TCM方剤の品質標準化の設計図を示す。
---
...
![図1. イオン化モードの選択と抽出条件の最適化。(A) 負イオン/正イオンモードのBPC比較、(B) 水・メタノール比を変えた抽出、(C) 抽出時間(30/60/90/120分)の比較。](assets/shenxiao-granules-2dlc-ms-qmarker-mdf/fig01-p6.png)
...
![図2. 2D-LCシステムの直交性評価。(A-C) 3種のHILIC（BEH Amide・XBridge Amide・Acchrom XAmide）とRP（HSS T3）の相対保持時間の散布図・線形回帰（Acchrom XAmide R²=0.25）、(D) Acchrom XAmide/HSS T3ペアの独立成分分析（ICA）。](assets/shenxiao-granules-2dlc-ms-qmarker-mdf/fig02-p8.png)
...
![図3. 1D HILICで得たSXG試料のクロマトグラムと、UHPLC-Q-TOF-MSで分析した6画分のBPC（1D分離を2Dで6フラクションに展開）。](assets/shenxiao-granules-2dlc-ms-qmarker-mdf/fig03-p10.png)
...
![図4. MDF支援によるSXGの化学的特性化。(A) モデル開発（質量欠損プロット）、(B) 実装（化学空間の濃縮）、(C) MDF境界式（7区分線形不等式）。](assets/shenxiao-granules-2dlc-ms-qmarker-mdf/fig04-p10.png)
...
![図5. 同一UHPLC-Q-TOF-MS条件下での8構成生薬（菟絲子・酒製五味子・地黄・黄耆・炮製淫羊藿・蓮子・丹参・炮製車前子）の指紋分析。](assets/shenxiao-granules-2dlc-ms-qmarker-mdf/fig05-p12.png)
...
![図6. 12 Q-markerの抽出イオンクロマトグラム（EIC）。(A) 空溶媒、(B) 混合標準液、(C) SXG試料抽出物。](assets/shenxiao-granules-2dlc-ms-qmarker-mdf/fig06-p13.png)
