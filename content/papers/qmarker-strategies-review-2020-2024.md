...
digest_chart_label: 戦略別の採用文献数（全60件）
digest_chart: [S1 機序駆動|22, S2 プロファイル効果|24, S3 in silico|8, S4 多基準|6]
summary: 伝統中薬(TCM)処方の品質マーカー(Q-marker)選定戦略について、2020–2024年にWeb of Science/PubMedで発表された60研究を系統的にレビューし、[S1]機序駆動型検証・[S2]プロファイル-効果相関モデリング・[S3]in silico事前フィルタリング・[S4]多基準意思決定の4戦略に分類、各戦略の事例(処方・標的・手順・同定Q-marker)を表化して長所短所を比較し、最適な統合アプローチを提案した総説の全訳(＋訳者補足)。4戦略の比較表と代表事例表を完全収載。
---
...
![図1. グラフィカルアブストラクト（4つのQ-marker同定戦略の概観）。](assets/qmarker-strategies-review-2020-2024/fig1.png)
...
![図2. PRISMA文献選定フロー（Web of Science 1638件＋PubMed 2234件＝計3872件→重複除去→タイトル/抄録スクリーニング2882件→全文評価95件→最終採用60件）。](assets/qmarker-strategies-review-2020-2024/fig2.png)
...
![図3. (A) 年別出版数（2020–2024）、(B) 戦略別の割合（S1 36.67%／S2 40.00%／S3 13.33%／S4 10.00%、計60件）、(C) 戦略別の平均Q-marker同定数（S1 7.23／S2 6.61／S3 8.25／S4 7.50）。](assets/qmarker-strategies-review-2020-2024/fig3.png)
...
![図4. 戦略1（機序駆動型検証）のワークフロー模式図（成分プロファイル→ネットワーク薬理でQ-marker候補→in vitro/in vivoアッセイ→含量確認）。](assets/qmarker-strategies-review-2020-2024/fig4.png)
...
![図5. 戦略2（プロファイル-効果相関）のワークフロー模式図（スペクトル-効果relationship／相関モデリング／BP人工ニューラルネットワーク）。](assets/qmarker-strategies-review-2020-2024/fig5.png)
...
![図6. 戦略4（多基準意思決定フレームワーク）の模式図（含量・薬効・特異性・安定性・配伍などの多基準をスコア化して統合）。](assets/qmarker-strategies-review-2020-2024/fig6.jpeg)
...
![図7. 本レビューが提案するQ-marker同定の統合戦略（血清/尿メタボロミクス＋PCMSモデリング→候補化合物/バイオマーカー→TCM原則に基づく多次元スコアリング）。](assets/qmarker-strategies-review-2020-2024/fig7.png)
