...
![本手法（AAL: 支援付き能動学習）の概念図——化合物＋物理化学情報＋履歴データから初期モデルを作り、実験実行→モデル更新→次実験の選択を繰り返す。](assets/assisted-active-learning-lc-method-development/graphical-abstract.png)
...
![図1. グラウンドトゥルース（真の）化合物保持モデル・応答曲面と、初期の化合物保持モデル・初期応答曲面の比較（A-D）。](assets/assisted-active-learning-lc-method-development/fig1.png)

![図2. (A) テストステロンの初期保持モデル（黒線＝現モデル、灰色＝不確かさ、黒点＝未使用の測定値）、(B) 初期獲得関数（acquisition）。](assets/assisted-active-learning-lc-method-development/fig2.png)
...
![図3. 試料Aの最初の3実験統合後の更新（勾配傾き22.5・8.18・15）。(A) 更新した保持モデル、(B) テストステロンの更新モデル、(C) 更新応答曲面、(D) 獲得関数。](assets/assisted-active-learning-lc-method-development/fig3.png)
...
![図4. 試料Bの最初の3実験後の更新（勾配傾き22.5・4.5・12.85）。(A) 更新した保持モデルほか。](assets/assisted-active-learning-lc-method-development/fig4.png)
