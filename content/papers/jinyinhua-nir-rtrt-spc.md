---
title: 近赤外分光法とリアルタイムリリース試験・統計的工程管理図を組み合わせた、生薬「金銀花」工業濃縮工程のオンライン品質管理への応用
slug: jinyinhua-nir-rtrt-spc
source_pdf: 1-s2.0-S1350449522001165-main.pdf
doi: https://doi.org/10.1016/j.infrared.2022.104135
level: practitioner
date: 2026-07-02
tags: [生薬, 分光法, QC・品質評価, 製造法]
digest_tagline: 気泡・流動の影響を除く自動NIR検出装置とPLSモデル、Shewhart／SPE管理図を組み合わせ、金銀花濃縮工程を工業規模でリアルタイムリリース試験する
digest_stats: [IF（2024）|3.8, 対象|金銀花(LJF)濃縮工程, モニタリング|NIR（AND装置）, 定量指標|NCA・CA・CCA・密度・水分, 検証バッチ|NOC13／AOC9]
digest_points: [気泡・固形不純物・流動の影響を除去する自動NIR検出(AND)装置を独自設計し静止スペクトル収集を実現, NCA・CA・CCA・密度・水分の5指標についてPLSモデルを構築しオンライン定量で良好な予測精度を確認, ShewhartとMSPC(SPE)管理図を組み合わせたRTRTでNOC／AOCバッチを判別しリリース基準を設定]
digest_chart_label: PLSモデルの予測誤差 RSEP（%）
digest_chart: [NCA|9.76, CA|1.96, CCA|7.41, 密度|0.68, 水分|1.31]
summary: 生薬「金銀花」の濃縮工程に対し、独自設計の自動NIR検出装置とPLSモデル、Shewhart／MSPC(SPE)管理図を組み合わせたリアルタイムリリース試験(RTRT)を工業規模で適用し、NCA・CA・CCA・密度・水分のオンライン定量とバッチ放出基準の設定を実証した研究の全訳。
---
...
![図1. AND制御システム。](assets/jinyinhua-nir-rtrt-spc/fig1.png)

![図2. AND装置の模式図。](assets/jinyinhua-nir-rtrt-spc/fig2.png)
...
![図3. 標準溶液(A)およびバッチNOC-1の代表的な終点サンプル(B)のHPLCクロマトグラム(1. NCA; 2. CA; 3. CCA)。](assets/jinyinhua-nir-rtrt-spc/fig3.png)
...
![図4. 典型的な濃縮工程（バッチNOC-10）におけるNCA(A)、CA(B)、CCA(C)、密度(D)、水分含量(E)の時間推移。リファレンス分析法による測定値。](assets/jinyinhua-nir-rtrt-spc/fig4.png)
...
![図5. LJF濃縮物の生NIRスペクトル(A)、および1st D・S-Gで前処理されたスペクトルデータ：5450〜6100 cm⁻¹ (B)；4500〜7500 cm⁻¹ (C)。](assets/jinyinhua-nir-rtrt-spc/fig5.png)
...
![図6. バッチ7・バッチ10の濃縮工程全体を通じたPCスコアの推移チャート(A)；NOC・AOCバッチの工程スペクトルデータのPC1 vs. PC2散布図(B)。](assets/jinyinhua-nir-rtrt-spc/fig6.png)
...
![図7. バッチNOC-13の濃縮工程における、オンラインNIRスキャンおよびリファレンス分析法によるNCA(A)、CA(B)、CCA(C)、密度(D)、水分含量(E)の時間推移。](assets/jinyinhua-nir-rtrt-spc/fig7.png)
...
![図8. NCA(A)、CA(B)、CCA(C)、密度(D)、水分含量(E)のShewhart管理図。](assets/jinyinhua-nir-rtrt-spc/fig8.png)
...
![図9. Hotelling T2管理図(A)およびSPE管理図(B)。管理図中の上方管理限界は95%管理限界。](assets/jinyinhua-nir-rtrt-spc/fig9.png)
