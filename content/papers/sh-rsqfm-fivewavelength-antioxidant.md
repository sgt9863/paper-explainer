...
![図1. 4階層の全体定量類似度からなる六面体(ROFP)概念図。W/R・P/C・Q/M・QF/MFの2指標平均で階層構造を可視化。](assets/sh-rsqfm-fivewavelength-antioxidant/fig1.png)

![図2. (A)条件別TQ値、(B)3Dクロマトグラム、(C)5波長融合プロセス、(D)(E)ROFP生成プロセス、(F)20バッチのROFP。](assets/sh-rsqfm-fivewavelength-antioxidant/fig2.png)

### RSQFMによる評価

22バッチを **6グレード** に分類(0.845 < Smr < 0.980、47.0% < Pmr < 112.3%)。不確かさパラメータは全て閾値0.2未満で頑健。単一波長データは情報が限られ、5波長統合のFWFFPがより信頼できる結果を与えた。

![図3. (A)5単一波長＋融合後のRSQFM評価結果(HPLC)、(B)クラスター解析ヒートマップ、(C)スコア散布図、(F)VIPプロット。](assets/sh-rsqfm-fivewavelength-antioxidant/fig3.png)

### 4成分定量（SCM vs RFPQM）

4成分(**クロロゲン酸・カフェ酸・ルチン・キレノール**)をSCMとRFPQMで定量し、両法に有意差なし（RFPQMの正確性を確認）。

![図4. (A)試料・標準溶液のHPLCクロマトグラム、(B)20バッチ中4成分の含量、(C-F)2測定法によるCA・CAA・RN・KLの近似曲線。](assets/sh-rsqfm-fivewavelength-antioxidant/fig4.png)
...
![図5. (A)UV特性吸収ピーク、(B)FTIR特性吸収ピーク、(C)異なる融合点でのUV量子化スペクトル、(D)FTIR量子化スペクトル。](assets/sh-rsqfm-fivewavelength-antioxidant/fig5.png)

![図6. (A)(B)HPLC・UV・FTIRで評価したSmr・Pmrのレーダーチャート、(C)Pmr-FWFFP・Pmr-UV・Pmr-FTIR・Pmr-Fの変動。](assets/sh-rsqfm-fivewavelength-antioxidant/fig6.png)
...
![図7. (A)IC50値、(B)スコア散布図、(C)(D)訓練/テストモデルのY実測値対Y予測値、(E)標準化回帰係数プロット、(F)VIPプロット。](assets/sh-rsqfm-fivewavelength-antioxidant/fig7.png)
