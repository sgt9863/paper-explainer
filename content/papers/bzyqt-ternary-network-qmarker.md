---
title: 定量的三元ネットワークに基づく漢方処方からのQ-marker探索 — 補中益気湯を例として
...
date: 2026-07-04
published: 2024
tags: [方剤・中成薬, ネットワーク薬理, Q-marker]
digest_tagline: 検出性・生物学的利用能・有効性の3属性をエントロピー重み法で統合した「三元ネットワーク」で補中益気湯のQ-markerを定量的に判別
digest_stats: [IF（2024）|8.8（要確認）, 対象|補中益気湯(BZYQT), 候補成分|17, Q-marker|9, 検証バッチ|36]
digest_points: [従来のQ-marker研究は複数属性を等重みで扱い重要度の重み付けを軽視していたという課題に対しエントロピー重み法(EWM)で検出性・生物学的利用能・有効性を統合する三元ネットワーク戦略を提案, BZYQTの34成分を定量し血漿・小腸内容物(SIC)中の17候補成分の動態とマクロファージ・脾リンパ球での免疫調節活性を評価して回帰面積により9成分をQ-markerに選定, 3社36バッチの補中益気丸(BZYQP)についてQ-marker由来フィンガープリントの類似度評価が従来の共通ピーク法より品質差異に高感度であることを実証]
digest_chart_label: 三元ネットワークの重み内訳（EWM算出値／％）
digest_chart: [検出性(testability)|49.747, 有効性(effectiveness)|26.086, 生物学的利用能(bioavailability)|24.166]
summary: 700年以上の歴史を持つ漢方処方・補中益気湯(BZYQT)について、成分含量（検出性）・血漿及び小腸内容物中の動態（生物学的利用能）・マクロファージ/脾リンパ球への免疫調節活性（有効性）の3属性をエントロピー重み法で統合する「定量的三元ネットワーク」を提案し、17候補成分から9のQ-markerを選定。これらを共通ピークとした補中益気丸(BZYQP、3社36バッチ)のHPLC-UVD-ELSDフィンガープリント類似度評価により、従来法より高感度な品質評価を実証した研究の全訳（+訳者補足）。
---
...
![図1. TCM標準湯剤からのQ-marker定量的発見と関連製品の全体的品質評価への応用のためのフローチャート。I.標準湯剤の調製と含量決定 → II.候補Q-markerのスクリーニング → III.in vitro免疫調節活性評価 → IV.EWMに基づく三元ネットワーク構築によるQ-marker発見（検出性49.747％・有効性26.086％・生物学的利用能24.166％）→ V.標準湯剤由来Q-markerの製品（BZYQP）全体品質評価への応用。](assets/bzyqt-ternary-network-qmarker/fig1.png)
...
![図2. BZYQT 6バッチにおける34化合物の含量分布（データは平均値±SDで表示）。トリテルペノイドサポニン類・フラボノイド類・テルペノイド類・その他に分類。](assets/bzyqt-ternary-network-qmarker/fig2.png)
...
![図5. BZYQTから選定された17候補Q-markerの化学構造（原文図キャプションは印刷上の欠落により「XXXX」となっている。図中の化合物名はカリコシン、カリコシン-7-O-β-D-グルコシド、ホルモノネチン、オノニン、リクイリチゲニン、リクイリチン、イソリクイリチゲニン、3,5,6,7,8,3′,4′-ヘプタメトキシフラボン、ヘスペリジン、ナリルチン、サイコサポニンA、シミゲノシド、18β-グリチルリチン酸、アストラガロシドIV、アストラガロシドII、アトラクチレノリドIII、マグノフロリン）。](assets/bzyqt-ternary-network-qmarker/fig5.png)
...
![図7. 17候補Q-markerの「検出性(testability)-有効性(effectiveness)-生物学的利用能(bioavailability)」三元ネットワーク（レーダーチャート）。頂点はDT（検出性）・DE（有効性）・DB（生物学的利用能）を表す。](assets/bzyqt-ternary-network-qmarker/fig7.png)
