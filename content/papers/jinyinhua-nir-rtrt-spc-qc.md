---
title: 近赤外分光法とリアルタイムリリース試験(RTRT)・統計的工程管理(SPC)チャートを用いた、生薬「金銀花」工業濃縮工程のオンライン品質管理
slug: jinyinhua-nir-rtrt-spc-qc
...
date: 2026-07-04
published: 2022
tags: [生薬, 分光法, 製造法, QC・品質評価]
digest_tagline: 金銀花の工業規模濃縮工程を、自動NIR測定装置(AND)＋PLSモデル＋SPC管理図でリアルタイムリリース判定
digest_stats: [IF（2024）|3.83, 対象|金銀花(Lonicerae Japonicae Flos)濃縮工程, 規模|3トン濃縮罐×6基(工業スケール), モデル|PLS(NCA・CA・CCA・密度・水分), SPC|Shewhart＋Hotelling T2／SPE]
digest_points: [気泡・固形不純物・流動の影響を除去する自動NIR検出装置(AND)を新規設計し、工業規模でオンライン静的スペクトル収集を実現, フェノール酸3成分(NCA・CA・CCA)と密度・水分の計5項目でPLSモデルを構築。予測誤差(RSEP)は0.68～9.76%と良好, リアルタイムリリース試験(RTRT)のリリース基準を提示。ShewhartチャートとSPEチャートは互いに補完し合い、異常バッチ(AOC)を検出できた]
digest_chart_label: 最終濃縮液の管理図中心線(Shewhart CL, mg/g)
digest_chart: [NCA|1.31, CA|11.72, CCA|2.07]
summary: 金銀花(Lonicerae Japonicae Flos)の工業規模濃縮工程を対象に、気泡・固形物・流動の影響を除去する自動NIR検出装置(AND)を新規設計し、近赤外分光法とリアルタイムリリース試験(RTRT)、統計的工程管理(SPC、Shewhart・Hotelling T2・SPE)を組み合わせてオンライン品質管理を行った研究(Infrared Physics & Technology 2022)の全訳。NCA・CA・CCA・密度・水分のPLSモデル、RTRTリリース基準値、異常バッチ検出結果を収載。
---
...
**キーワード**
近赤外分光法、忍冬（スイカズラ）の花、濃縮工程、リアルタイムリリース試験、統計的工程管理
...
![図1. AND制御システム全体構成（自動制御センター・NIRワークステーション・AND装置の3部構成）。](assets/jinyinhua-nir-rtrt-spc-qc/fig1-and-control-system.png)

![図2. AND装置の模式図（濃縮罐と一体化された気泡・固形物除去用フィルタ、循環ポンプ、スペクトル・試料採取モジュール(SSCM)の詳細）。](assets/jinyinhua-nir-rtrt-spc-qc/fig2-and-device-schematic.png)
...
![図8. NCA (A)、CA (B)、CCA (C)、密度 (D)、水分含量 (E) のShewhartチャート（実測値■・予測値○、UCL/CL/LCLを表示。サンプル1～27がAOCバッチ、28～36がNOCバッチ）。](assets/jinyinhua-nir-rtrt-spc-qc/fig8-shewhart-charts.png)
...
![図9. Hotelling T2チャート (A) とSPEチャート (B)。管理チャート中の上方限界は95%管理限界。赤丸=AOCバッチ、黒丸=NOCバッチ。](assets/jinyinhua-nir-rtrt-spc-qc/fig9-hotelling-spe-charts.png)
