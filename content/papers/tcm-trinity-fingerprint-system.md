...
![Fig.1 標準品とRFPのHPLCクロマトグラム(A)、異なる融合時間窓でのパラメータ変化(B)、SQFMによるS1の3波長・融合波長指紋(C)、THz試験の品質検査(D)。](assets/tcm-trinity-fingerprint-system/fig1.jpg)

### 4.2 6成分の定量分析

#### 4.2.1 MAML定量分析

MAMLがQAMSに代わって甘草の活性成分をより正確に定量できるかを検討するため、外部標準法(ESM)・QAMS・MAMLの3法を用いて6物質の濃度を測定した（Supplementary Table S3）。ESM[35]では、定量成分含量はTable 1に示す線形回帰式から容易に算出される。GLAをMEMAとし、残り5化合物の含量はZhimin Wangら[36]によるQAMSに従って算出した。2.2節のMAML原理に基づき、GLAを同様にMEMAとして用いた。まず、*R*は95.5%を上回った（Table 2）。次にILG・LG・IQA・LQ・LQAの相対補正係数を算出した。
...
![Fig.2 産地・生育様式別の6成分含有率の積み上げグラフ(A)、6成分に基づくPCAスコア散布図・ローディングプロット(B)。](assets/tcm-trinity-fingerprint-system/fig2.jpg)
...
![Fig.3 UV(A)/THz(B)の量子化点の検討、85バッチのUV指紋(C)、産地別UV量子指紋(D)、産地別THz量子指紋(E)、産地別HPLC指紋(F)、UV量子指紋またはHPLCスペクトルの薬効相関結果(G)。](assets/tcm-trinity-fingerprint-system/fig3.jpg)
