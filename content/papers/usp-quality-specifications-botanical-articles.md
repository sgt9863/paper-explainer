---
title: 米国薬局方（USP）による生薬（植物由来品）の品質規格：モノグラフ7セクションの技術要件と実例
slug: usp-quality-specifications-botanical-articles
source_pdf: d0ccabc4-1s2.0S0944711318301144main.pdf
doi: https://doi.org/10.1016/j.phymed.2018.04.014
drive_url: (未アップロード：ユーザー返却PDF)
level: researcher
date: 2026-07-08
published: true
tags: [生薬, 規制・薬事, QC・品質評価, 指紋分析, 多成分定量, レビュー]
digest_tagline: USP／HMCの生薬モノグラフはTitle・Definition・Identification・Composition・Contaminants・Specific tests・Additional requirementsの7部構成――HPLC/HPTLCの実例で品質規格を解説
digest_stats: [IF|8.81（Phytomedicine・JCR2024・Q1）, 発行元|米国薬局方協会（USP）栄養補助食品・生薬部門, 構成|生薬モノグラフ7セクション, 同定|直交する2試験以上（HPLC＋HPTLC＋形態）, 汚染物質|重金属4元素・農薬70種・アフラトキシン・微生物]
digest_points: [USP（薬局方本体・DS）とHMC（伝統生薬）の生薬モノグラフはTitle/Definition/Identification/Composition/Contaminants/Specific tests/Additional requirementsの7セクションで同一性・純度・含量を規定, 同定は形態・組織学に加えHPLC/HPTLCの特徴的フィンガープリントで基原種と混同種（近縁種すり替え）を判別――桂皮・金銀花・紅景天・何首烏の実例で解説, 定量は生物活性マーカー優先でNLT指定・単一標準品＋換算係数で複数成分を定量／汚染物質はヒ素カドミウム鉛水銀のPDE・農薬70種・アフラトキシンB1≤5ppb等を規格化]
digest_chart_label: USP生薬モノグラフの同定試験（直交性の担保）
digest_chart: [最低限含める直交同定試験の数|2, 何首烏の混同種判別で試験したPolygonum近縁種の数|5]
summary: 米国薬局方協会（USP）が定める、生薬（植物由来品・生薬系サプリメント・伝統生薬）の科学的品質規格を包括解説するレビュー。USP本体・国民医薬品集（NF）・生薬集（HMC）の各モノグラフは、Title（表題）・Definition（定義）・Identification（同定）・Composition＝assay（含量）・Contaminants（汚染物質）・Specific tests（固有試験）・Additional requirements（追加要件）の7セクションからなり、同一性・純度・含量で全体的な品質を特性評価する。同定はHPLCとHPTLCの特徴的フィンガープリントで近縁種のすり替えや混入を検出し、定量は生物活性マーカー成分をバリデート済みクロマト法で測定、純度規格は重金属・農薬・カビ毒（アフラトキシン）等の汚染物質を制限する。桂皮（Cinnamomum cassia）・金銀花（Lonicera japonica）・紅景天（Rhodiola）・何首烏（Polygonum multiflorum）・当帰（Angelica sinensis）の5実例で技術要件を具体的に示す。
---
