...
図（Fig.1–9）はPyMuPDFで原文PDFから抽出し本文中に埋め込み済み。
-->
...
![Figure 1. 分析的QbD（AQbD）の構成要素。中心の「AQbD」を核に、ATP（分析ターゲットプロファイル）同定→重要分析手順属性の同定→DoEによる方法最適化・開発→MODR（設計空間）→管理戦略・リスク評価→AQbD法バリデーション→継続的方法モニタリング→（ATPへ還流）という循環的ワークフローを示す。](assets/aqbd-ich-q14-analytical-procedure-review/fig1.png)
...
![Figure 2. 従来の分析法開発の一般的ワークフロー。標的アナライトの特性把握→文献検索・用途定義→OFAT（一因子ずつ）による初期方法条件の決定と最適化→試料調製と分析法条件の確定→方法開発の最終化→方法バリデーション→方法移管→再バリデーション・定型分析、という直線的な流れを示す。](assets/aqbd-ich-q14-analytical-procedure-review/fig2.png)

![Figure 3. AQbDアプローチの一般的プロセス。原薬合成経路・製剤QTPPの情報収集→文献検索・CQA・ATP同定と初期リスク評価→リスク評価とCMV（重要方法変数）の特定→DoEによるスクリーニング／最適化→MODRの確立と検証→管理戦略の策定→MODRから導いた目標方法のバリデーション→継続的方法モニタリング、という循環的・反復的な流れを示す（Fig.2のOFAT型直線フローと対比）。](assets/aqbd-ich-q14-analytical-procedure-review/fig3.png)
...
![Figure 4. ICH/USPによる方法性能特性の分類。「特異性・直線性・正確性」は系統的変動（systemic variability）、「検出限界・定量限界・精度」は固有のランダム変動（random variability）に分類される一方、「範囲」「堅牢性」はICH/USPの分類上「該当なし（not applicable）」と位置づけられていることを示す。](assets/aqbd-ich-q14-analytical-procedure-review/fig4.png)
...
![Figure 5. USP〈1220〉に基づく分析ターゲットプロファイル（ATP）の構造とライフサイクル全体での活用。ATPが分析手順ライフサイクルのStage 1（手順設計・開発：知識収集→リスク評価・管理→分析管理戦略→再現戦略）、Stage 2（手順性能適格性確認：適格性確認試験の設計・実施→報告書）、Stage 3（継続的手順性能検証：日常モニタリング・傾向分析→継続的改善→管理戦略）の各段階にどう関わるかを示す。](assets/aqbd-ich-q14-analytical-procedure-review/fig5.png)

![Figure 6. 分析手順開発における製品・工程ライフサイクル管理の考慮点（出典: Lúcia Volta e Sousaら, 2021の図を一部改変）。ATPを起点に分析法ライフサイクルが「許容範囲を伴う報告値」に収斂し、そこからライフサイクル管理・リスク管理・事前知識・継続的改善を経て設計理解→性能適格性確認→継続的性能検証の3段階へ展開する構造を示す。](assets/aqbd-ich-q14-analytical-procedure-review/fig6.jpeg)
...
![Figure 7. 確立条件（EC）に関する分析手順の承認後変更の判断フロー。「分析法性能を確保するため当該変数を管理する必要があるか」から始まり、Yesの場合は「ECである／ECでない」に分岐、ECであれば変更時のリスク（重要性評価と分析管理戦略を踏まえた「高」または「中／低」）を判定し、高リスクなら追加の管理策の有無に応じて事前承認または届出へ、中／低リスクなら届出へ、ECでない場合は報告不要へと至る意思決定樹を示す。](assets/aqbd-ich-q14-analytical-procedure-review/fig7.png)
...
![Figure 8. リアルタイム試験の分析手順（原論文キャプション: "Analytical procedures for Real-Time Testing"、HIL＝Hardware-in-the-loop、SIL＝Software-in-the-loop、PIL＝Processor-in-the-loopと注記）。左側に「モデルベース設計」（製品要件分析⇄ユニットの統合）→「リアルタイム試験」（モデリング・シミュレーション⇄HILシミュレーション）の流れ、右側に「ラピッドコントロールプロトタイピング⇄SIL/PILシミュレーション」を示す。原図は制御工学分野で一般的なモデルベース設計・HIL検証のフローを転用したもので、本文が説明する分析法のRTRT（工程内・原薬/製剤のリアルタイム品質評価）そのものを描いた図ではない点に注意（原論文の図がそのまま流用されている）。](assets/aqbd-ich-q14-analytical-procedure-review/fig8.png)
...
![Figure 9. ICH Q14構成要素と他のICH文書との統合。中心の「分析手順開発の文書上の考慮（ICH Q14）」を核に、時計回りに「ATP・重要分析手順属性・分析手順パラメータ範囲」→「ICH Q6A/Q6B（リアルタイムリリース試験）」→「AQbD：最小／拡張アプローチ」→「ICH Q10（医薬品品質システム：堅牢性と分析パラメータ範囲）」→「M4Q(R1)（CTD）」→「ICH Q2（方法性能バリデーション）」→「ICH Q9（品質リスク管理と知識評価）」→「ICH Q12（技術・規制上の考慮）」という循環的な統合関係を示す。](assets/aqbd-ich-q14-analytical-procedure-review/fig9.png)
...
- 図（Fig.1〜9、特にFig.9のICH文書統合図）は本文中に埋め込み済み。なおFig.8（リアルタイム試験）は原論文自体が制御工学分野のモデルベース設計／HIL検証の一般図を転用したもので、パイプラインQCの実態を直接描いたものではない（本文注記参照）。
