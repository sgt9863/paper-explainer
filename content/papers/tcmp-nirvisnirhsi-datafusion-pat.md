---
title: 中薬顆粒(TCMP)の重要品質特性を非破壊予測するFT-NIRとVis/NIR-HSIのデータフュージョン戦略の開発
slug: tcmp-nirvisnirhsi-datafusion-pat
source_pdf: 1-s2.0-S0924203125000141-main.pdf
doi: https://doi.org/10.1016/j.vibspec.2025.103780
level: practitioner
date: 2026-07-02
tags: [方剤・中成薬, 分光法, ケモメトリクス, 製造法]
digest_tagline: FT-NIRとVis/NIR-HSIをデータフュージョンし中薬顆粒(TCMP)の安息角・粒度・水分をPAT platform上で非破壊予測、高位融合(HLF)が全指標で最高精度を達成
digest_stats: [IF（2024）|2.7, 対象|中薬顆粒(TCMP) 20バッチ, 手法|FT-NIR＋Vis/NIR-HSI, 最良融合|高位融合(HLF), 外部検証|10バッチ・40試料]
digest_points: [FT-NIRは水分・粒度予測で優位、Vis/NIR-HSIは安息角(流動性)予測で優位という相補性を確認, 低位(LLF)・中位(MLF)・高位(HLF)の3段階データフュージョンを比較し高位融合が全CQAで最高精度, HLFモデルは安息角RPD5.090・粒度RPD18.893・水分RPD30.891を達成し10バッチの外部検証でも高い予測精度を維持]
digest_chart_label: HLF(高位融合)モデルの予測精度RPD（3つの重要品質特性）
digest_chart: [安息角|5.090, 粒度|18.893, 水分|30.891]
summary: 中薬顆粒(TCMP)の重要品質特性(安息角・粒度・水分含量)をFT-NIRとVis/NIR-HSIのデータフュージョンで非破壊予測するPATプラットフォームを開発し、低位・中位・高位の3融合戦略を比較、高位融合(HLF)が最も高精度であることを外部検証まで含めて示した研究の全訳。
---
...
Yac = b + K1X1,pc + K2X2,pc  (3)
Ydfpc = b + K1X1,pvc + K2X2,pvc  (4)
Ydfpv = b + K1X1,pvv + K2X2,pvv  (5)
...
i) Vis/NIR-HSIモデルと比較して、FT-NIRモデルは粒度・水分含量の予測でより高い精度を示した一方、Vis/NIR-HSIモデルは安息角の予測でより優れた性能を示した。
ii) LLF・MLFモデルは、粒度・水分含量の予測においてVis/NIR-HSIモデルより優れていたが、FT-NIRモデルほどの精度には達しなかった。
iii) MLFモデルは全指標の予測精度を高め、特に粒度予測での改善が顕著であった。
iv) FT-NIRとVis/NIR-HSIデータを統合したHLFモデルは、全指標において最高の精度を達成した。安息角・粒度・水分含量のRPD値は、単一モデルおよびLLF/MLFモデルと比較して有意に増加した。
