---
title: FT-NIRとVis/NIR-HSIのデータ融合戦略による中薬顆粒(TCMP)の重要品質特性の非破壊予測
slug: tcmp-ftnir-hsi-fusion-cqa
...
date: 2026-07-03
published: 2025
tags: [方剤・中成薬, 分光法, ケモメトリクス, QC・品質評価, 製造法]
digest_tagline: FT-NIRとVis/NIR-HSIを高次融合(HLF)し、中薬顆粒(TCMP)の水分含量・粒度・安息角(流動性)を高精度に非破壊予測するPATプラットフォーム
digest_stats: [IF|3.15（Vibrational Spectroscopy・JCR2024集計値／要確認）, 対象|中薬顆粒(TCMP・流動層造粒), バッチ数|20（検量75／予測25サンプル・外部検証10バッチ40サンプル）, 融合戦略|低次・中次・高次(LLF／MLF／HLF), 最良RPD|水分含量30.891・粒度18.893・安息角5.090(いずれもHLF)]
digest_points: [流動層造粒で調製した20バッチの中薬顆粒(TCMP)をFT-NIRとVis/NIR-HSIで計測しPLS予測モデルを構築, 単独計測ではFT-NIRが水分含量・粒度で優位、Vis/NIR-HSIが安息角(流動性)で優位という相補性を確認, 低次・中次・高次(LLF／MLF／HLF)のデータ融合を比較し高次融合(HLF)が3指標すべてで最高精度を達成しオンラインPAT基盤として提案]
digest_chart_label: 安息角(repose angle)予測モデルのRPD比較（単独計測 vs 融合戦略、本文3.4の直接比較値）
digest_chart: [FT-NIR|2.968, Vis/NIR-HSI|3.365, LLF|3.558, MLF|3.835, HLF|5.090]
summary: 中薬（中医薬）顆粒(TCMP)の造粒工程における重要品質特性(CQA)—流動性(安息角)・粒度・水分含量—を、FT-NIR単独／Vis/NIR-HSI単独に加え低次・中次・高次のデータ融合戦略で予測するPLSモデルを比較検討した論文の全訳。流動層造粒で作った20バッチ(検量75・予測25サンプル)にFT-NIRとVis/NIR-HSIを適用し、化学的特徴に強いFT-NIRと空間・物性情報に強いVis/NIR-HSIの相補性を活かした高次融合(HLF)モデルが全指標で最高精度(RPD: 安息角5.090／粒度18.893／水分含量30.891)を達成、外部検証(10バッチ40サンプル)でも高い予測精度を維持し、オンラインでの非破壊品質モニタリングPATプラットフォームとしての可能性を示した。
---
...
- 本論文は、生薬そのものの品質評価(Q-marker探索や成分指紋分析)というよりも、中薬エキス顆粒(TCMP)の**製造工程(流動層造粒)における重要品質特性(CQA)**を、分光法主体のPAT(プロセス分析工学)手法でリアルタイム予測しようとする、製造・工程管理寄りの論文である。他の多くの収載論文（HPLC指紋・Q-marker系）とは着眼点が異なり、「製造法」「QC・品質評価」の観点から関連性が高い。
