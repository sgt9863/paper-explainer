---
title: ネットワーク薬理学に基づく抗血栓生薬の品質マーカー(Q-marker)研究 — 丹参・川芎薬対を例として
slug: antithrombotic-herbs-networkpharm-qmarker
...
digest_tagline: 化合物クラスター網と血栓機能モジュール網を統合したBMCTネットワークで丹参・川芎薬対のQ-marker9成分を選定
digest_stats: [IF（2024）|6.54, 対象|丹参・川芎(DS-CX)薬対, 手法|ネットワーク薬理＋HPLC＋トロンビン阻害試験, Q-marker|9成分, 抗血栓標的|18標的(FM1/FM2/FM3)]
digest_points: [化合物の構造類似性によるCCネットワークと血栓関連機能モジュール(FM)ネットワークを結合したBMCT(bilayer modularized compound target)ネットワークを構築, 「二段階」アルゴリズムでCC1・CC2・CC13を重要クラスターと同定し丹参6成分＋川芎3成分の計9成分をQ-markerに選定, in vitroトロンビン阻害試験でタンシノンI・タンシノンIIAの活性を確認しHPLC定量で9成分が両生薬中の主要成分であり配合比により含量が変動することを実証]
digest_chart_label: 丹参:川芎=1:1配合時の9マーカー成分含量(mg/g)
digest_chart: [クロロゲン酸|0.50, フェルラ酸|1.39, ロズマリン酸|2.07, サルビアノール酸B|8.26, サルビアノール酸A|0.17, コニフェリルフェルレート|2.18, クリプトタンシノン|0.67, タンシノンI|1.12, タンシノンIIA|0.61]
summary: 丹参(Salvia miltiorrhiza)・川芎(Ligusticum chuanxiong)の抗血栓生薬ペアを例に、化合物の構造クラスター網と血栓関連機能モジュール網を統合したBMCT(bilayer modularized compound target)ネットワークを構築し、「二段階」アルゴリズムで9種のQ-marker候補を選定、in vitroトロンビン阻害試験とHPLC含量定量で妥当性を検証したネットワーク薬理研究の全訳(＋訳者補足)。
---
...
![Figure 1. 本研究の研究フレームワーク（化合物層のCCネットワークと血栓標的層のFMネットワークをBMCTネットワークに統合し、二段階アルゴリズムでQCマーカーを同定、化学プロファイリングとトロンビン阻害試験で検証する一連の流れ）。](assets/antithrombotic-herbs-networkpharm-qmarker/fig01-framework.jpeg)
...
行列Ep×qで示される化学モジュールと生物学的モジュールの関係について、マーカーを同定するために式(4)に示す二段階の解法を用いた。argmaxは関連関数の定義域における、関数値が最大となる点を表す。第一段階では、{P(m*v)、v=1,…,q}集合、すなわちm*vを規制するCCの組合せを、最も強い連結、すなわち最も高いeuvの値で同定した。有意なCCをスクリーニングした後、第二段階として、潜在的QCマーカーを特定した。Q(m*v)をm*vを担う潜在的QCマーカーとする。Q(m*v)は本研究における関連する制限条件を満たす複数のciであった。{Q(m*v)、v=1,…,q}集合は、それぞれ異なる関連生体分子を規制するQCマーカーの組合せであった。最終的に、w(ci)の値が最も高い潜在的QCマーカーを得た。血栓症の複雑性と生物学的ネットワークにおけるスモールワールドの特性を鑑みると、この種の生体分子モジュールに基づく戦略はQCマーカーの同定に妥当であった。
...
![Figure 4. 構造類似性に基づくDS-CXのCCネットワーク。円形ノードは化合物を、エッジはタニモト係数0.6超の類似性を示す。黄色ノードはDSの化合物、緑色ノードはCXの化合物、灰色ノードはDSとCXの共通化合物を表す。異なるクラスターは異なる母骨格を意味する。](assets/antithrombotic-herbs-networkpharm-qmarker/fig04-cc-network.jpeg)
...
![Figure 5. BMCTネットワーク。上層ノードはCCを表し、ノード脇の番号はクラスター番号を示す。黄色は下層への直接的な連結を持つ部分、青色は連結を持たない部分を表す。灰色ノードは非クラスター化単一化合物の集合を表す。下層ノードは血栓に関連するタンパク質／タンパク質複合体を表す。青はFM1、黄はFM2、赤はFM3、緑はFM1&2、紫はFM1&3を表す。灰色エッジは個々のタンパク質／タンパク質複合体間の相互作用を、上下層間のエッジは化合物-標的間の相互作用を表す。二層間のエッジの太さは連結強度を、ノードの大きさはそのエッジの総強度に比例する。](assets/antithrombotic-herbs-networkpharm-qmarker/fig05-bmct-network.jpeg)
...
![Figure 7. 標準品混合物(A)、DS:CX=1:1(B)、DS(C)、CX(D)のHPLCクロマトグラム。1: クロロゲン酸、2: フェルラ酸、3: ロズマリン酸、4: サルビアノール酸B、5: サルビアノール酸A、6: コニフェリルフェルレート、7: クリプトタンシノン、8: タンシノンI、9: タンシノンIIA。](assets/antithrombotic-herbs-networkpharm-qmarker/fig07-hplc-chromatograms.jpeg)
...
<sup>a</sup> クロロゲン酸・フェルラ酸・コニフェリルフェルレートの含量はCXの生薬量を基準に、ロズマリン酸・サルビアノール酸B・サルビアノール酸A・クリプトタンシノン・タンシノンI・タンシノンIIAの含量はDSの生薬量を基準に算出。
<sup>b</sup> 「–」は検出限界未満（未検出）。
<sup>c</sup> 平均値±標準偏差。
