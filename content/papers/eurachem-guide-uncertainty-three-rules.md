...
![図2 容量測定における不確かさの評価。合成不確かさ $u_c$ への寄与——(1) ガラス器具の規格限界、(2) 標線まで充填する際のばらつき、(3) 周囲温度の影響。左「General case（文献5）」はガイドの一般的な扱いで、3成分を独立に足し合わせるため $u_c$ が最も大きくなる。中「Standard volumetric glassware」は規定許容差を用いる標準的な容量器具の場合で、成分(2)は成分(1)にすでに包含されている（円2が円1の内側に描かれている）ため実質2成分となり $u_c$ が小さくなる。右「Volumetric glassware individually calibrated」は個別校正した容量器具の場合で、成分(1)が消えて成分(2)と(3)のみとなり $u_c$ は最も小さい](assets/eurachem-guide-uncertainty-three-rules/fig2-volumetric-uncertainty.png)
