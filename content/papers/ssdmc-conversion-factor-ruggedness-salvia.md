---
title: 単一標準による多成分同時定量（SSDMC）における変換係数（conversion factor）の頑健性・堅牢性——丹参のタンシノン3成分を例に
slug: ssdmc-conversion-factor-ruggedness-salvia
source_pdf: 7c91798a-1s2.0S0021967311008739main.pdf
doi: https://doi.org/10.1016/j.chroma.2011.06.058
level: practitioner
date: 2026-07-23
published: 2011
tags: [生薬, QC・品質評価, QAMS, 多成分定量, HPLC・UPLC, ケモメトリクス]
digest_tagline: 丹参のタンシノン3成分をtanshinone IIA 1本で定量するSSDMC/変換係数の「実験室間でのばらつき」を頑健性・堅牢性試験で解剖し、許容幅と鍵パラメータ（UV波長）を特定
digest_stats: [IF（2024）|約4.1, 対象|丹参(Danshen)のタンシノン3成分, 手法|SSDMC(単一標準多成分定量)／変換係数, 単一標準|tanshinone IIA, 許容幅|FC 1.15-1.24／FI 1.23-1.36]
digest_points: [tanshinone IIAを唯一の標準とし変換係数FC(cryptotanshinone)=1.18／FI(tanshinone I)=1.31でDanshenの3成分を同時定量——外部標準法と総含量RSD<1%で一致, 頑健性(nested-factorial)試験で装置がFCに(P<0.001)・カラムがFIに(P=0.001)最も効き堅牢性(OVAT＋Plackett-Burman)試験でUV波長がFCの最大要因・傾き感度(積分)がFIの最大要因と特定, 変換係数は必ず変動するが鍵パラメータ(波長・ピーク測定・標準液濃度)を管理すれば許容幅(FC 1.15-1.24／FI 1.23-1.36)に収まりSSDMCは薬局方手順として実験室間展開が可能]
digest_chart_label: 変換係数の許容幅（頑健性・堅牢性試験より、総含量RSD<2.0%の条件）
digest_chart: [FC下限|1.15, FC上限|1.24, FI下限|1.23, FI上限|1.36]
summary: 生薬・漢方の品質管理で有望な「単一標準による多成分同時定量（SSDMC）」を実験室間で広く使うには、成分ごとの応答差を補正する変換係数（conversion factor＝相対応答係数RRF）のばらつきが障害になる。本研究は丹参（Danshen）のタンシノン3成分（tanshinone IIA・cryptotanshinone・tanshinone I）をtanshinone IIA 1本で定量する系を例に、頑健性（環境要因）と堅牢性（操作要因＋ピーク測定パラメータ）を体系的に試験し、UV波長・ピーク測定パラメータ・標準液濃度が変換係数を左右する鍵であること、許容幅内なら実験室間で使えることを示した研究の全訳（＋訳者補足）。
---
...
![図1（原論文 Fig.1）. tanshinone I・cryptotanshinone・tanshinone IIA の化学構造とUVスペクトル。cryptotanshinone（C, C19H20O3, MW 296.36, λmax=264 nm）・tanshinone I（I, C18H12O3, MW 276.29, λmax=245 nm）・tanshinone IIA（A, C19H18O3, MW 294.34, λmax=269 nm）。いずれもジテルペンキノンで骨格は類似するが極大吸収波長が異なる。](assets/ssdmc-conversion-factor-ruggedness-salvia/fig1.png)
...
![図3（原論文 Fig.3）. 標準溶液濃度が変換係数に及ぼす影響。(A)0〜約500 µg/mLの全域、(B)0〜40 µg/mLの拡大。tanshinone I（●）は濃度上昇とともに変換係数が低下し変化が顕著、cryptotanshinone（▲）は10 µg/mL超で安定。両者とも10 µg/mL未満で著しく上昇する。](assets/ssdmc-conversion-factor-ruggedness-salvia/fig3.png)
...
![図2（原論文 Fig.2）. (A)混合標準溶液と(B)試料溶液（山東産丹参）の代表的HPLCクロマトグラム。C=cryptotanshinone・I=tanshinone I・A=tanshinone IIA。理論段数が高く未知物質から容易に分離できる。](assets/ssdmc-conversion-factor-ruggedness-salvia/fig2.png)
...
![図4（原論文 Fig.4）. 堅牢性試験の結果。(A)8因子各水準での cryptotanshinone の変換係数（線は平均値）、(B)同 tanshinone I、(C)各因子で+1水準から−1水準を引いた変換係数の最大差。X軸略号は表2参照。波長(WL)がFC・FIともに最大の効果を示す。](assets/ssdmc-conversion-factor-ruggedness-salvia/fig4.png)
...
![図5（原論文 Fig.5）. ピーク測定パラメータ試験の結果。(A)4因子各水準での cryptotanshinone の変換係数、(B)同 tanshinone I、(C)各因子で+1水準から−1水準を引いた変換係数の最大差。X軸略号は表3参照。FCはスリット/バンド幅に、FIは傾き感度(積分)に敏感。](assets/ssdmc-conversion-factor-ruggedness-salvia/fig5.png)
...
![図6（原論文 Fig.6）. Plackett–Burman計画の結果。(A・B)変換係数への因子効果の half-normal確率プロット（臨界効果ME・SMEを同定）、(C・D)同 Paretoチャート。cryptotanshinone（FC）では D_WL（波長）、tanshinone I（FI）では K_SS（傾き感度）が最大効果。](assets/ssdmc-conversion-factor-ruggedness-salvia/fig6.png)
...
1. United States Pharmacopeia Dietary Supplements Compendium, 2009, p. 1522.
2. National Commission of Chinese Pharmacopoeia, Technological Requirements of Research and Drafting Monograph of Traditional Chinese Medicine in Ch.P. 2010, 2008.
3. National Commission of Chinese Pharmacopoeia, Pharmacopoeia of People's Republic of China, vol. I, China Medical Science Press, Beijing, 2010, p. 36.
4. National Commission of Chinese Pharmacopoeia, Pharmacopoeia of People's Republic of China, vol. I, China Medical Science Press, Beijing, 2010, p. 243.
5. United States Pharmacopeia 33-NF 28, vol. I, US Pharmacopoeial Convention, Rockville, MD, 2010, p. 1046.
6. United States Pharmacopeia 33-NF 28, vol. I, US Pharmacopoeial Convention, Rockville, MD, 2010, p. 1037.
7. United States Pharmacopeia 33-NF 28, vol. I, US Pharmacopoeial Convention, Rockville, MD, 2010, p. 1093.
8. X.Y. Gao, Y. Jiang, J.Q. Lu, P.F. Tu, *J. Chromatogr. A* 1216 (2009) 2118.
9. National Commission of Chinese Pharmacopoeia, Pharmacopoeia of People's Republic of China, vol. I, China Medical Science Press, Beijing, 2010, p. 285.
10. A.H. Liu, Y.H. Lin, M. Yang, H. Guo, S.H. Guan, J.H. Sun, D.A. Guo, *J. Chromatogr. B* 846 (2007) 32.
11. Z.H. Shi, J.T. He, T.T. Yao, W.B. Chang, M.P. Zhao, *J. Pharm. Biomed. Anal.* 37 (2005) 481.
12. L.M. Zhou, M. Chow, Z. Zuo, *J. Pharm. Biomed. Anal.* 41 (2006) 744.
13. Y. Liu, X.R. Li, Y.H. Li, L.J. Wang, M. Xue, *J. Pharm. Biomed. Anal.* 53 (2010) 698.
14. B. Dejaegher, Y. Vander Heyden, *J. Chromatogr. A* 1158 (2007) 138.
15. D.C. Montgomery, Design and Analysis of Experiments, 6th ed., Posts & Telecom Press, Beijing, 2009, p. 459.
16. C.A. Beasley, J. Shaw, Z. Zhao, R.A. Reed, *J. Pharm. Biomed. Anal.* 37 (2005) 559.
17. Y. Vander Heyden, A. Nijhuis, J. Smeyers-Verbeke, B.G.M. Vandeginste, D.L. Massart, *J. Pharm. Biomed. Anal.* 24 (2001) 723.
