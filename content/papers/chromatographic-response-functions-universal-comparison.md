...
![図2. カテゴリーI（必要理論段数Nの最小化）の真の最適勾配条件と代表的CRFの等高線図。(a)絶対的真値、(b-f)各CRF（白矢印＝各CRFの最良判定領域）。](assets/chromatographic-response-functions-universal-comparison/fig2.png)

![図3. カテゴリーII（必要分析時間の最小化）の真の最適勾配条件と代表的CRFの等高線図。(a)絶対的真値、(b-f)各CRF。](assets/chromatographic-response-functions-universal-comparison/fig3.png)
